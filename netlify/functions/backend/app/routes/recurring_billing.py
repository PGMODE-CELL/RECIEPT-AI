...
@router.post("/{org_id}")
async def create_plan(org_id: int, contact_id: int = None, name: str = "", frequency: str = "monthly", interval_count: int = 1, total_amount: float = 0, items: str = "[]", tax_rate_id: int = None, start_date: str = None, max_cycles: int = None, auto_send: str = "yes", notes: str = "", db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
...
@router.get("/{org_id}")
async def list_plans(org_id: int, status: str = None, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    stmt = select(RecurringBillingPlan).filter(RecurringBillingPlan.org_id == org_id)
    if status:
        stmt = stmt.filter(RecurringBillingPlan.status == status)
    plans = (await db.execute(stmt.order_by(RecurringBillingPlan.created_at.desc()))).scalars().all()
...
async def generate_invoice(org_id: int, plan_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    plan = (await db.execute(select(RecurringBillingPlan).filter(RecurringBillingPlan.id == plan_id, RecurringBillingPlan.org_id == org_id))).scalar_one_or_none()
    if not plan:
    ...
    inv_count = (await db.execute(select(func.count()).select_from(Invoice).filter(Invoice.org_id == org_id))).scalar()
...
async def toggle_plan(org_id: int, plan_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    plan = (await db.execute(select(RecurringBillingPlan).filter(RecurringBillingPlan.id == plan_id, RecurringBillingPlan.org_id == org_id))).scalar_one_or_none()
    if not plan:
        raise HTTPException(404, "Plan not found")
