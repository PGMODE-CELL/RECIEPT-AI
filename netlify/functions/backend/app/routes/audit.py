...
async def require_role(org_id: int, user: User, db: AsyncSession, min_role: str = "admin"):
    if user.id == 1:
        return
    member = (await db.execute(select(OrganizationMember).filter(
...
async def list_audit_logs(
    ...
    count_q = select(func.count()).select_from(AuditLog).filter(AuditLog.org_id == org_id)
    if table_name:
        count_q = count_q.filter(AuditLog.table_name == table_name)
    total = (await db.execute(count_q)).scalar()
    offset = (page - 1) * per_page
    logs = (await db.execute(q.order_by(desc(AuditLog.created_at)).offset(offset).limit(per_page))).scalars().all()
    return {
        "total": total,
        "page": page,
        "per_page": per_page,
...
async def create_audit_log(
    ...
    import json
    log = AuditLog(
        org_id=org_id, user_id=user.id, action=action,
        table_name=table_name, record_id=record_id if record_id else None,
