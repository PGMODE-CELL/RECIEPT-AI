...
const TASK_COLORS = ["#3b82f6", "#8b5cf6", "#10b981", "#f59e0b", "#ef4444", "#ec4899", "#06b6d4", "#f97316"];
  ...
  const addTask = () => {
    ...
    setTasks([
      ...effectiveTasks,
      {
        ...
        color: TASK_COLORS[effectiveTasks.length % TASK_COLORS.length],
      },
    ]);
    setDialogOpen(false);
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Visual timeline with dependencies and critical path
            </p>
          </div>
        ...
        <div className="flex gap-4 text-sm">
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-red-500" /> Critical Path
          </div>
          <div className="flex items-center gap-1">
            <Flag className="w-3 h-3 text-amber-500" /> Milestone
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-blue-400" /> Task
          </div>
          <div className="flex items-center gap-1">
            <div className="w-px h-4 bg-red-500 border-dashed" /> Today
          </div>
        </div>
        ...
        <Card className={`overflow-hidden ${effectiveTasks.length === 0 ? "hidden" : ""}`}>
              ...
              <div className="flex-1 overflow-x-auto">
                <div className="flex h-10 border-b bg-gray-50 dark:bg-gray-900 sticky top-0">
                  {dates.map((d, i) => (
                    <div
                      key={d}
                      className={`flex-shrink-0 border-r px-1 flex flex-col items-center justify-center text-xs ${
                        i === todayIdx ? "bg-blue-100 dark:bg-blue-900 font-bold" : ""
                      }`}
                  ...
                  {effectiveTasks.map(task => {
                    ...
                    return (
                      <div key={task.id} className="border-b relative" style={{ height: rowHeight }}>
                        {todayIdx >= 0 && (
                          <div
                            className="absolute top-0 bottom-0 w-px bg-red-400 border-l border-dashed z-10"
                            style={{ left: todayIdx * dayWidth }}
                          />
                        )}
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div
                              className={`absolute top-1.5 rounded cursor-pointer transition-all ${
                                isOnCritical ? "ring-2 ring-red-500" : ""
                              }`}
                              style={{
                                left: style.left,
                                width: style.width,
                            ...
                            >
                              <div
                                ...
                                style={{ width: `${task.progress}%`, backgroundColor: "rgba(0,0,0,0.2)" }}
                              />
