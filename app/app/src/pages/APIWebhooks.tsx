...
// No webhook delivery-log endpoint exists yet; start empty rather than seeding demo logs.
...
export default function APIWebhooks() {
  ...
  const testWebhook = () => {
    setTestResult(null);
    toast.info("Live webhook test delivery is not available yet.");
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Event subscriptions, payload customization, and delivery logs
          </p>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 max-h-40 overflow-y-auto border rounded p-3">
                    {availableEvents.map(event => (
                      <label key={event} className="flex items-center gap-2 text-sm cursor-pointer">
                        <input
                          type="checkbox"
                          checked={newSub.events.includes(event)}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {Math.round(logs.reduce((s, l) => s + l.duration, 0) / logs.length)}ms
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <TabsContent value="subscriptions">
          <div className="space-y-4">
            ...
            {!isLoading && subscriptions.length === 0 && (
              <div className="text-center py-8 text-gray-500">No webhook subscriptions yet</div>
            )}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {sub.events.map(e => (
                          <Badge key={e} className="bg-indigo-100 text-indigo-700 text-xs">
                            {e}
                          </Badge>
                        ))}
                      </div>
                ...
                <TableBody>
                  {logs.map(log => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        <Badge className="bg-indigo-100 text-indigo-700 text-xs">{log.event}</Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          {log.status === "failed" && (
                            <Button size="sm" variant="ghost" onClick={() => retryWebhook(log.id)}>
      ...
      <Dialog open={testDialogOpen} onOpenChange={() => setTestDialogOpen(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Webhook Testing Sandbox</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                defaultValue={JSON.stringify(
                  {
                    event: "invoice.paid",
                    data: { id: "INV-TEST-001", amount: 1500.0, customer: "Test Corp" },
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">#{selectedLog.attempt}</p>
                </div>
                <div>
