...
async def stripe_webhook(
    ...
    if not stripe or not STRIPE_WEBHOOK_SECRET:
        # Fail closed: never trust unsigned webhook payloads.
    ...
    if event_type == "payment_intent.succeeded" or event_type == "payment.success":
        pay = (
            await db.execute(
                select(Payment).filter(Payment.gateway_payment_id == pi_id)
            )
        ).scalar_one_or_none()
        if pay:
            ...
            inv = (
                await db.execute(select(Invoice).filter(Invoice.id == pay.invoice_id))
            ).scalar_one_or_none()
            if inv:
