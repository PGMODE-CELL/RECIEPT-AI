...
@router.post("/{org_id}")
def create_period(org_id: int, name: str = "", start_date: str = None, end_date: str = None, is_fiscal_year: str = "no", notes: str = "", db: Session = Depends(get_db), user=Depends(get_current_user)):
...
@router.get("/{org_id}")
def list_periods(org_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    periods = db.query(AccountingPeriod).filter(AccountingPeriod.org_id == org_id).order_by(AccountingPeriod.start_date.desc()).all()
...
def close_period(org_id: int, period_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    period = db.query(AccountingPeriod).filter(AccountingPeriod.id == period_id, AccountingPeriod.org_id == org_id).first()
    if not period:
...
def open_period(org_id: int, period_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    period = db.query(AccountingPeriod).filter(AccountingPeriod.id == period_id, AccountingPeriod.org_id == org_id).first()
    if not period:
        raise HTTPException(404, "Period not found")
