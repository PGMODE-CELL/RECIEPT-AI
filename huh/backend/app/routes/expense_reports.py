...
@router.post("/{org_id}")
def create_report(org_id: int, title: str = "", description: str = "", db: Session = Depends(get_db), user=Depends(get_current_user)):
...
@router.get("/{org_id}")
def list_reports(org_id: int, status: str = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
    q = db.query(ExpenseReport).filter(ExpenseReport.org_id == org_id)
    if status:
        q = q.filter(ExpenseReport.status == status)
    reports = q.order_by(ExpenseReport.created_at.desc()).all()
...
def get_report(org_id: int, report_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def add_line(org_id: int, report_id: int, date: str = None, category: str = "", amount: float = 0, description: str = "", billable: str = "no", project_id: int = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
    report = db.query(ExpenseReport).filter(ExpenseReport.id == report_id, ExpenseReport.org_id == org_id).first()
    if not report:
        raise HTTPException(404, "Report not found")
...
def submit_report(org_id: int, report_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    report = db.query(ExpenseReport).filter(ExpenseReport.id == report_id, ExpenseReport.org_id == org_id).first()
    if not report:
        raise HTTPException(404, "Report not found")
...
def approve_report(org_id: int, report_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    report = db.query(ExpenseReport).filter(ExpenseReport.id == report_id, ExpenseReport.org_id == org_id).first()
    if not report:
        raise HTTPException(404, "Report not found")
...
def reject_report(org_id: int, report_id: int, notes: str = "", db: Session = Depends(get_db), user=Depends(get_current_user)):
    report = db.query(ExpenseReport).filter(ExpenseReport.id == report_id, ExpenseReport.org_id == org_id).first()
    if not report:
        raise HTTPException(404, "Report not found")
